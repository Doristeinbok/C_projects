...
typedef int (*cmp_func_t)(void *curr_data, void *requested_data); /* SUCCESS = 0, FAIL = 1*/
...
extern hash_t *HashCreate(hash_func_t hash_func, cmp_func_t cmp_func, size_t hash_size, const void *param); 
...
extern void *HashFind(hash_t *hash, const void *data);

/* Execute operation on each element in table */
extern int HashForEach(hash_t *hash, hash_action_func_t action_func, const void *param);
